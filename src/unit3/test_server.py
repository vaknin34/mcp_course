...
try:
    from server import analyze_file_changes, get_pr_templates, mcp, suggest_template
...
class TestAnalyzeFileChanges:
    ...
    @pytest.mark.asyncio
    async def test_includes_required_fields(self) -> None:
        ...
        with patch("subprocess.run") as mock_run:
            ...
            if is_implemented:
                ...
                assert any(key in data for key in ["files_changed", "files", "changes", "diff", "changed_files"]), (
...
class TestGetPRTemplates:
    ...
    async def test_returns_json_string(self) -> None:
        ...
        # For starter code, accept error messages; for full implementation, expect dict
        is_implemented = not ("error" in data and isinstance(data, dict))
        if is_implemented:
            assert isinstance(data, dict), "Should return a JSON object containing templates"
            assert "templates" in data, "Should have a 'templates' key"
        else:
            # Starter code - just verify it returns something structured
            assert isinstance(data, dict), "Should return a JSON object even if not implemented"

    @pytest.mark.asyncio
    async def test_returns_templates(self) -> None:
        ...
        if is_implemented:
            assert "templates" in templates, "Should have a 'templates' key"
            template_dict = templates["templates"]
            assert len(template_dict) > 0, "Should return at least one template"

            # Check that templates have expected structure
            for template_name, template_content in template_dict.items():
                assert isinstance(template_content, str), "Each template should be a string"
...
class TestSuggestTemplate:
    ...
    async def test_returns_json_string(self) -> None:
        ...
        result = await suggest_template("Fixed a bug in the authentication system", "bug")
    ...
    @pytest.mark.asyncio
    async def test_suggestion_structure(self) -> None:
        """Test that the suggestion has expected structure."""
        result = await suggest_template("Added new feature for user management", "feature")
...
class TestToolRegistration:
    ...
    def test_tools_have_decorators(self) -> None:
